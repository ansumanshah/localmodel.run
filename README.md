...
Built as a programmatic-SEO content site: one statically-rendered page per
`model × device`, per device, and per model, so it ranks in Google and is easy
for ChatGPT / Gemini / Claude to crawl and cite.
