/*
  Paint-shape adapter (historical filename from the gauge adapter era) — the single
  ...
  on `window.LmGauge` for genuinely interactive surfaces via buildGauge().
...
/**
 * Expose a client-side window.LmGauge over the real data + engine. Only call
 ...
 */
export function buildGauge(models: ModelRow[], devices: DeviceRow[]) {
