...
export function useCasesFor(m: ModelRow): UseCase[] {
  ...
  const out: UseCase[] = []
  if (m.subtype === 'coder' || /coder|devstral|(^|[^a-z])code([^a-z]|$)/.test(s)) out.push('coding')
  if (/(^|[^a-z])r1([^a-z]|$)|reasoning|magistral|qwq|thinking|nemotron/.test(s)) out.push('reasoning')
