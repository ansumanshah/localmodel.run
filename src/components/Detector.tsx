...
// Grouped dropdown options: models cluster into <optgroup>s by the lab that made
// them (makers alphabetical, size ascending within) so the 45 fine-grained
// families collapse to ~15 readable groups; devices group by category in a
// logical order (Apple Silicon, GPUs, laptop, phones), memory ascending within.
type OptGroup = { label: string; options: { id: string; label: string }[] };

// Concise maker label for grouping: reuse the icon-system maker resolver, then
// drop any ", by X" qualifier (e.g. "Qwen, by Alibaba" -> "Qwen").
const makerLabel = (family: string) => makerForFamily(family).label.replace(/,\s*by\s.+$/i, "");
...
const MODEL_GROUPS: OptGroup[] = groupBy(
  [...models].sort((a, b) => a.params_b - b.params_b),
  (m) => makerLabel(m.family || "Other"),
)
  .sort((a, b) => a[0].localeCompare(b[0]))
  .map(([maker, ms]) => ({
    label: maker,
    options: ms.map((m) => ({ id: m.id, label: `${m.name} (${m.params_b}B)` })),
  }));

const DEVICE_GROUPS: OptGroup[] = groupBy(
  [...devices].sort((a, b) => a.memory_gb - b.memory_gb),
  (d) => d.category || "other",
)
  .sort((a, b) => (CAT_ORDER[a[0]] ?? 9) - (CAT_ORDER[b[0]] ?? 9))
  .map(([cat, ds]) => ({
    label: CAT_LABEL[cat] ?? cat,
    options: ds.map((d) => ({ id: d.id, label: d.name })),
  }));
...
// Select field: a styled native <select> (accessible, id-based, the source of
// truth) faced as a .field with the selected label shown and a custom chevron.
  ...
  return (
    <div className="det-body">
      ...
      <div className="mt-3 flex items-center gap-3">
        <button type="button" onClick={detect} className="btn text-xs">
